...
def Simulate_Orifice_Emptying(tank_with_orifice, model_types = ["isothermal"]):
    
    # COPV_with_orifice = TankWithOrifice(
    #                                     tank_name = "COPV",
    #                                     tank_pressurant_initial_pressure = parameters.COPV_starting_pressure,
    #                                     tank_volume = parameters.COPV_volume,
    #                                     orifice_nominal_size = "0.010",
    #                                     orifice_test_flow_rate = 8.12/60, # [SCFM]
    #                                     orifice_test_pressure = (80 * c.PSI2PA) + c.ATM2PA,
    #                                    )
    
    # orifice_0010_flow_rate = 0.1 # [SCFM] 
    # orifice_0010_flow_rate = 8.12/60 # [SCFM] 
    # orifice_flow_rate_test_pressure_gauge = (80 * c.PSI2PA) + c.ATM2PA

    tank_with_orifice.orifice_flow_coefficient = Cv_Choked_from_SCFM(parameters.pressurant_name, tank_with_orifice.orifice_test_flow_rate, tank_with_orifice.orifice_test_pressure, c.T_AMBIENT)
    print(f"orifice {tank_with_orifice.orifice_nominal_size}: {tank_with_orifice.orifice_flow_coefficient:.5f} Cv")

    dt = 0.05

    fig, ax = plt.subplots(2, 2, sharex=False) # share x-axis for cleaner look
    
    for model_type in model_types:

        initial_time = 0
        pressurant_initial_mass_flow_rate = 0 # [kg/s]
        pressurant_initial_temperature = 273 # [k]
        pressurant_initial_pressure = tank_with_orifice.tank_pressurant_initial_pressure
        pressurant_initial_density = PropsSI("D", "T", pressurant_initial_temperature, "P", pressurant_initial_pressure, parameters.pressurant_name)
        pressurant_initial_mass = tank_with_orifice.tank_volume * pressurant_initial_density
        pressurant_initial_entropy = PropsSI("S", "T", pressurant_initial_temperature, "P", pressurant_initial_pressure, parameters.pressurant_name)
        ...
        while pressurant_current_pressure > (15 * c.PSI2PA):
            
            pressurant_current_mass_flow_rate = Calculate_Choked_Mass_Flow_from_Cv(parameters.pressurant_name, tank_with_orifice.orifice_flow_coefficient, pressurant_current_pressure, pressurant_current_temperature)
            
            new_time = time_array[-1] + dt
            pressurant_new_mass = pressurant_mass_array[-1] - (pressurant_current_mass_flow_rate * dt)
            pressurant_new_density = pressurant_new_mass / tank_with_orifice.tank_volume
            
            if model_type == "isentropic":
                pressurant_new_entropy = pressurant_initial_entropy
                pressurant_new_temperature = PropsSI("T", "S", pressurant_new_entropy, "D", pressurant_new_density, parameters.pressurant_name)
                pressurant_new_pressure = PropsSI("P", "S", pressurant_new_entropy, "D", pressurant_new_density, parameters.pressurant_name)
            elif model_type == "isothermal":
                pressurant_new_temperature = pressurant_current_temperature
                pressurant_new_pressure = PropsSI("P", "T", pressurant_new_temperature, "D", pressurant_new_density, parameters.pressurant_name)
        ...
        ax[0,0].legend(model_types)
        ax[0,0].grid()

        ax[0,1].plot(time_array, pressurant_mass_flow_rate_array)
        ax[0,1].set_ylabel(f"mass flow rate [kg]")
        ...
        fig.suptitle(f"Emptying {tank_with_orifice.tank_name} through {tank_with_orifice.orifice_nominal_size} orifice using {model_type} model")
...
def main():
    ...
    class TankWithOrifice:
        # General Parameters
        tank_name: str
        tank_pressurant_initial_pressure: float
        tank_volume: float
        orifice_nominal_size: str
        orifice_test_flow_rate: float
        orifice_test_pressure: float
        orifice_flow_coefficient: float = None
    ...
    oxidizer_tank_with_orifice = TankWithOrifice(
                                        tank_name = "Liquid Oxygen Tank",
                                        tank_pressurant_initial_pressure = parameters.largest_possible_tank_pressure,
                                        tank_volume = parameters.oxidizer_tank_usable_volume,
                                        # orifice_nominal_size = "0.010",
                                        # orifice_test_flow_rate = 8.12/60, # [SCFM]
                                        # orifice_test_pressure = (80 * c.PSI2PA) + c.ATM2PA,
                                        orifice_nominal_size = "0.125",
                                        orifice_test_flow_rate = 1263/60, # [SCFM]
                                        orifice_test_pressure = (80 * c.PSI2PA) + c.ATM2PA,
                                       )
    
    # Simulate_Orifice_Emptying(COPV_with_orifice)
    Simulate_Orifice_Emptying(oxidizer_tank_with_orifice, model_types=["isothermal", "isentropic"])


if __name__ == "__main__":
    # main(model_types = ["isentropic", "isothermal"])
    main()
