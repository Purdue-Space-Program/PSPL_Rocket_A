...
def main(property_to_use = "density"):
    ...
    #     # V_ullage_fu = V_FU * ULLAGE_FU # [m^3] fuel tank ullage volume
    ...
    elif vehicle_name == "Rocket_A":
        ADIABATIC = False # True to ignore heat transfer in the tanks
        PREPRESS = "isothermal" # prepressurization type (choose from 'adiabatic' or 'isothermal') isothermal means infinite loiter time

        PRESS_LINE_CHILL = False # True to account for heat transfer in a pressurant line that runs through the oxidizer tank
        PRESS_GAS = "nitrogen"
        ...
        # Fuel
        FUEL = "ethanol" # used since coolprop doesn't have ipa, assumption i made is that they are similar enough in relevant fluid properties - david
    ...
    if PREPRESS == "adiabatic": # Calculate initial conditions assuming adiabatic prepress
        ...
        P0_copv = P_COPV
        T0_copv = T_COPV
    elif PREPRESS == "some bullshit": # Calculate initial conditions assuming tank ullage temperatures equal tank temperatures (infinite loiter time)
