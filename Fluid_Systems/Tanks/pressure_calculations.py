...
def Calculate_Pressure(parameters): # this is a separate function for reasons i dont remember but its important...
    ...
    parameters.unfreeze()
    parameters.maximum_tank_pressure_to_account_for_droop = parameters.nominal_tank_pressure / droop_factor # the set pressure is higher than the nominal tank pressure to account for droop that occurs when the run valves open and there is flow through the regulator
    
    relief_valve_set_pressure = Round_Up_To_Nearest_Multiple(original_value = (parameters.maximum_tank_pressure_to_account_for_droop - c.ATM2PA), 
    ...
    parameters.maximum_allowable_tank_pressure = relief_valve_set_pressure * 1.1 # relief valves are rated for full flow when they are pressurized at 10% over their set pressure, one thing i am not sure about is if it is 10% over the relief pressure in PSIA or PSIG but the difference in pressure for this in a relief valve at a set pressure of 550 is about 13.5 PSI
    parameters.hydroproof_tank_pressure = parameters.maximum_allowable_tank_pressure * parameters.hydroproof_factor # this is the exact pressure to hydroproof at to prove that the tanks can handle the pressure that the relief valves enforce the tanks to be
    parameters.freeze()

    output_table_data = []
    
    output_table_data.append(("nominal_tank_pressure", f"{parameters.nominal_tank_pressure * c.PA2PSI:.2f} PSIA"))
    output_table_data.append(("maximum_tank_pressure_to_account_for_droop", f"{parameters.maximum_tank_pressure_to_account_for_droop * c.PA2PSI:.2f} PSIA"))
