...
class VehicleParameters:
    ...
    largest_possible_tank_pressure: float = (25 * c.PSI2PA) * round(maximum_tank_pressure_to_acount_for_droop / (25 * c.PSI2PA)) # relief valves sold in increments of 25 psi
    ...
    # masses
    parachute_mass: float = 2.86875 * c.LBM2KG  # [kg] 168-inch https://www.the-rocketman.com/products/rocketman-high-performance-cd-2-2-parachutes    
...
def Setup_Vehicle_Parameters(show_plots = False, print_outputs = False, print_components = False, print_components_in_known_sections = False):
    ...
    if print_components_in_known_sections == True:
    ...
    if print_components == True:
        for component in wet_mass_distribution:
            ...
            if (print_components_in_known_sections == False) or (not ((component.name == "injector") or ((component.name == "lower_fuel_bulkhead") or (component.name == "upper_fuel_bulkhead")) or ((component.name == "lower_oxidizer_bulkhead") or (component.name == "upper_oxidizer_bulkhead")))):
    ...
    return(parameters, wet_mass_distribution, dry_mass_distribution)

def main():
    parameters, wet_mass_distribution, dry_mass_distribution = Setup_Vehicle_Parameters(show_plots = False, print_outputs = False, print_components = False, print_components_in_known_sections = False)
    return(parameters, wet_mass_distribution, dry_mass_distribution)

if __name__ == "__main__":
    Setup_Vehicle_Parameters(show_plots = True, print_outputs = True, print_components = True, print_components_in_known_sections = False)
else:
    parameters, wet_mass_distribution, dry_mass_distribution = Setup_Vehicle_Parameters()
    
    
    # fix this later when its not 2 am
